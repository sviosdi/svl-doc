...
#include ["Signal.hpp"](https://gitlab.com/sviosdi/signals_slots/-/blob/main/include/Signal.hpp)<br />
#include ["Slot.hpp"](https://gitlab.com/sviosdi/signals_slots/-/blob/main/include/Slot.hpp)
...
void connect(OBJ_APPELE * obj_appele, RT(OBJ_APPELE::* methode )(Params...)) {
    ...
    SlotKey key(reinterpret_cast<void*>(obj_appele), reinterpret_cast<char*>(&methode));
...
2. On construit un `SlotMethode` sans aucun argument : c'est une sorte de coquille vide, mais qui récupère tous les types
   énoncés précédemment.
3. On crée un `SlotKey` qui par contre lui va récupérer les données et les anonymiser.
4. On enregistre dans le signal (dans son map `connectedSlots`) la paire `SlotKey` ➞ `SlotMethode`. Et on peut le faire, car un
...
5. On teste ensuite si l'objet connecté est une instance d'une classe dérivée de la classe `svl::Object` et si c'est le cas, on ajoute le signal au
   ...
   signal et de la même façon _tous_ les signaux qui lui sont connectés.
...
```cpp
template <typename OBJ_APPELE, typename RT>
void disconnect(OBJ_APPELE* obj_appele, RT(OBJ_APPELE::* methode )(Params...)) {
    SlotKey key(reinterpret_cast<void*>(obj_appele), reinterpret_cast<char*>(&methode));
    connectedSlots.erase(key);
    if(std::is_base_of<Object, OBJ_APPELE>::value){
        ((Object*)obj_appele)->removeSignal(this);
    }
}
```
...
qui peut s'exécuter très rapidement.
...
void disconnect(void* obj_an) override {
    typename std::map<SlotKey, Slot<Params...>>::iterator itr = connectedSlots.begin();
...
```cpp
Object::~Object() {
  for(svl::SaSSignal* signal : connectedSignals) {
...
2. À la connexion d'un slot : `signal.connect(&a, &A::m)`
   ...
   - Si le slot se rattache à un objet dérivant de la classe `Object`:
     ...
     - Ajouter le signal à `connectedSignals` **même s'il est déjà présent**. (important!) [→ voir NOTE 2](#note_2)
...
3. À la déconnexion d'un slot : `signal.disconnect(&a, &A::m)`
   ...
   - On retire le signal de `connectedSignals`, _une seule fois_! (important!) [→ voir NOTE 2](#note_2)
...
:::note note 2
Pour ce dernier point, il n'est pas nécessaire de
:::
